...
def check_services_and_get_lb_url(db: Session):
    ...
    try:
        ...
        lb_node = next((node for node in registered_nodes if node.service_type == "load_balancer"), None)

        # If any required service is missing or the load balancer is not found, return False and None
        if not all(service in running_services for service in REQUIRED_SERVICES) or lb_node is None:
            return False, None
